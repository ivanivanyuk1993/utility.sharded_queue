...
fn crossbeam_queue_push_and_pop_concurrently(operation_count: usize) {
...
fn sharded_queue_push_and_pop_concurrently(operation_count: usize) {
    let max_concurrent_thread_count = available_parallelism().unwrap().get();
...
fn queue_mutex_push_and_pop_concurrently(operation_count: usize) {
    let max_concurrent_thread_count = available_parallelism().unwrap().get();
...
fn criterion_benchmark(criterion: &mut Criterion) {
    ...
    for operation_count in operation_count_list {
        for bench_fn_and_name in bench_fn_and_name_list.iter() {
            ...
            criterion.bench_function(&format!("{bench_fn_name}_{operation_count}"), |bencher| {
                bencher.iter(|| bench_fn(black_box(operation_count)))
            });
        }
    }
}
